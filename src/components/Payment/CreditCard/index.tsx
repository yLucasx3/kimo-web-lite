...
const CreditCard = ({ orderAmount }) => {
    ...
    useEffect(() => {
        const cardForm = (window as any).mp.cardForm({
                ...
                onCardTokenReceived: (error, token) => {
                    if (error) {
                        for (const err of error) {
                            ...
                            // card number invalid
                            if (code === 'E301') {
                                errorMessage('Numéro do cartão inválido.');
                                break;
                            }
                            // card expirationinvalid
                            if (code === '325' || code === '326') {
                                errorMessage('Vencimento do cartão inválido.');
                                break;
                            }
                            ...
                            if (code === '316') {
                                errorMessage('Nome impresso no cartão inválido.');
                                break;
                            }

                            // card number empty
                            if (code === '205') {
                                errorMessage('Número do cartão não pode ser vazio.');
                                break;
                            }

                            // card expiration empty
                            if (code === '208' || code === '209') {
                                errorMessage('Vencimento não pode ser vazio.');
                                break;
                            }

                            // security code empty
                            if (code === '224') {
                                errorMessage('Código de segurança não pode ser vazio.');
                                break;
                            }

                            // // cardholder name empty
                            if (code === '221') {
                                errorMessage('Nome impresso no cartão não pode ser vazio.');
                                break;
                            }
        ...
        <Container>
            <form id="form-checkout">
                <div className="input-with-icon">
                    ...
                    {thumbnail && <img src={thumbnail} className="thumbnail" alt="Bandeira do cartão de crédito" />}
                </div>
                <input type="hidden" name="cardExpirationMonth" id="form-checkout__cardExpirationMonth" />
                <input type="hidden" name="cardExpirationYear" id="form-checkout__cardExpirationYear" />
                ...
                <div className="group-inputs">
                    ...
                    />
                    <input type="text" name="securityCode" id="form-checkout__securityCode" maxLength={3} />
                </div>
                <select name="installments" id="form-checkout__installments"></select>
                <input type="hidden" name="cardholderEmail" id="form-checkout__cardholderEmail" />
                <select name="issuer" style={{ display: 'none' }} id="form-checkout__issuer"></select>
                <select
                    name="identificationType"
                    style={{ display: 'none' }}
                    id="form-checkout__identificationType"
                ></select>
                <input type="hidden" name="identificationNumber" id="form-checkout__identificationNumber" />
